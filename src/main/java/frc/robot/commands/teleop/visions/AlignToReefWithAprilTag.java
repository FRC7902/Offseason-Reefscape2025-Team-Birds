...
public class AlignToReefWithAprilTag extends Command {
  ...
  public AlignToReefWithAprilTag(CameraInterface m_autoAlignCamera, ReefSide triggerPressed, SwerveSubsystem swerveSubsystem) {
  ...
  public void initialize() {
    ...
    currentRobotPose = swerveSubsystem.getPose();
  ...
  public void execute() {
    ...
    if (hasLargeRotationDifference) {
      swerveSubsystem.alignRobotToAprilTag(aprilTagRotation, 0, 0);
    }
    else if (!hasLargeRotationDifference) {
      swerveSubsystem.alignRobotToAprilTag(aprilTagRotation, getDriverControllerLeftY(), -ySpeed);
    }
  }
